...
class _RemoveBackgroundPageState extends State<RemoveBackgroundPage> {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (!kIsWeb) {
          ...
          snapshotPath = '${tempDir.path}/remove_bg_${DateTime.now().millisecondsSinceEpoch}.png';
        ...
        final history = EditHistory(
          ...
          operationParameters: {'operation': 'remove_bg'},
        ...
        await widget.onUpdateImage(
          ...
          operationType: 'background',
          parameters: {'operation': 'remove_bg', 'historyId': historyId},
        );
  ...
  Future<Uint8List> _callClipDrop(Uint8List imageBytes) async {
    final localizations = AppLocalizations.of(context);
    try {
      final uri = Uri.parse('https://clipdrop-api.co/remove-background/v1');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(localizations?.removeBackgroundTitle ?? 'Remove Background'),
        leading: IconButton(
      ...
      body: Center(
        child: _isLoading
          ...
          children: [
            ...
            Text(
              localizations?.removingBackground ?? 'Removing background...',
      ...
      floatingActionButton: FloatingActionButton(
        ...
        tooltip: localizations?.pickImageTooltip ?? 'Pick Image',
